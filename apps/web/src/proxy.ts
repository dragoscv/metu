...
export const config = {
  matcher: [
    '/((?!_next/|favicon.ico|sign-in|docs|download|\\.well-known|api/auth|api/webhooks|api/inngest|api/health|api/companion|api/sdk/v1|api/internal|api/calendar|api/voice|api/push/vapid-public-key|api/dev/e2e-login|api/oauth/token|api/oauth/userinfo|api/oauth/revoke|api/oauth/device|api/oauth/jwks|api/oauth/.well-known|.*\\.(?:png|jpg|jpeg|svg|webp|ico|gif|woff2?|map)$).*)',
  ],
};
