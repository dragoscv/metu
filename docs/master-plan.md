...
| 6   | Companion advanced (v1.5)              | window tracking, clipboard, file watcher, wake-word, Ollama tunnel                                                                                                                      | **partial** — frontend clipboard ring + Porcupine wake-word adapter shipped; window-tracker + file-watcher Rust sensors with FE bridge + settings UI now landed; openWakeWord runner + Ollama tunnel still pending |
| 7   | notai + mmo wired up                   | both apps consume @metu/sdk, register as OAuth clients                                                                                                                                  | **partial** — notai consumes @metu/sdk; mmo not yet scaffolded                                                                                                                                                     |
...
| 10  | Audit & polish pass                    | settings/profile edit + delete, NDJSON workspace export, autonomy monthly soft-brake + FORCE_ASK for telegram/email, Sentry node init across hub/worker/mcp-server, protocol+auth tests | **shipped**                                                                                                                                                                                                        |
